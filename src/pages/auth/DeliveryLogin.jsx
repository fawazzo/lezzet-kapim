...
const DeliveryLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post('/api/auth/delivery/login', { email, password });
      login(data);
  ...
  return (
    <div className="flex justify-center items-center py-10">
      <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md">
        {/* Başlık Çevirisi */}
        <h2 className="text-3xl font-bold text-center text-primary-dark mb-6">Teslimatçı Girişi</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="w-full bg-primary-dark hover:bg-primary-dark/90 text-white font-bold py-2 px-4 rounded-lg transition duration-200 shadow-md"
          >
        ...
        <p className="mt-6 text-center text-gray-600">
          {/* Metin Çevirisi */}
          Yeni misiniz?{' '}
          <Link to="/delivery/register" className="text-primary-dark hover:underline font-medium">
